...
class GenThreePlaneMoMo(Optimizer):
    """
    Three-Plane MoMo
    ...
    def _step_network(self, loss: Optional[Union[float, torch.Tensor]], log_dict: Optional[Dict] = None):
        ...
        for group in self.param_groups[1:]:
            ...
            if group["tp_barf1"] != barf1:
                raise ValueError("All parameter groups must share the same tp_barf1 for network-wide GenThreePlaneMoMo.")
            if group["tp_barf2"] != barf2:
                raise ValueError("All parameter groups must share the same tp_barf2 for network-wide GenThreePlaneMoMo.")
            if group["tp_gamma1"] != gamma1:
                raise ValueError("All parameter groups must share the same tp_gamma1 for network-wide GenThreePlaneMoMo.")
            if group["tp_gamma2"] != gamma2:
                raise ValueError("All parameter groups must share the same tp_gamma2 for network-wide GenThreePlaneMoMo.")
            if group["tp_step"] != stored_tp_step:
                raise ValueError("All parameter groups must share the same tp_step for network-wide GenThreePlaneMoMo.")
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if precond == "adam":
                    ...
                    denom_m2_precond_inv_m2 += torch.sum(m2 * precond_t_inv_flattened * m2).item()
                    inner_m1_precond_inv_m2 += torch.sum(m1_for_update * precond_t_inv_flattened * m2).item()
                    ...
                    denom_m2_precond_inv_m2 += torch.dot(m2.flatten(), m2.flatten()).item()
                    inner_m1_precond_inv_m2 += torch.dot(m1_for_update.flatten(), m2.flatten()).item()
        ...
        try:
            ...
            # Reisdual tolerance score 
            # tol = 1e-8 * (1 + ||rhs|| + ||A|| * ||lambda||)
            ...
            if interior_solve_success:
                ...
                # If rank = 1 and Solvable => base solution + any vector in the kernel space of A
                    ...
                    # We extract the right singular vector v = [null_v1, null_v2]
                    # corresponding to the zero/smallest singular value
                    # In the 2x2 rank-1 case, this single vector spans Ker(A)
                    # Solution set: S = {[lambda1, lambda2] + t * [null_v1, null_v2] | [null_v1, null_v2] \in Ker(A)}
                    # where t = free scalar parameter that parameterizes all solutions along the kernel space direction
                    null_v1 = float(interior_null_vec[0].item()) # null_v1 entry
                    null_v2 = float(interior_null_vec[1].item()) # null_v2 entry
                    # lambda1 = lambda1 + t * null_v1
                    # lambda2 = lambda2 + t * null_v2
                    # lambda3 = 1 - lambda1 - lambda2
                    # lambda3 = 1 - (lambda1 + t * null_v1) - (lambda2 + t * null_v2)
                    #         = 1 - lambda1 - lambda2 - (t * null_v1) - (t * null_v2)
                    #         = lambda3 - (t * null_v1) - (t * null_v2)
                    #         = lambda3 - t(null_v1 + null_v2)
                    #         = lambda3 + t(- (null_v1 + null_v2))
                    # Hence, the third entry of the simplex direction is -(null_v1 + null_v2)
                    null_v3 = -(null_v1 + null_v2)

                    # Set upper and lower limit of free scalar parameter as unbounded
                    t_lower = float("-inf")
                    t_upper = float("inf")

                    # Defining the exploration rule by varying t in the right direction
                    # Since lambdai + t * null_vi > 0, to remain feasible
                    # => If null_vi > 0: t > (- lambdai)/ null_vi => we update the new lower bound: (-lambdai)/ null_vi
                    # => If null_vi < 0: t < (- lambdai)/ null_vi => we update the new upper bound: (-lambdai)/ null_vi
                    # => else null_vi = 0:
                    #     this coordinate does not change as t varies.
                    #     If lambda_i > 0, it imposes no bound on t.
                    #     If lambda_i <= 0, then no t can make this coordinate positive.
                    ...
                    # Get all the t scalar parameter bound
                    ...
                    if ok1 and ok2 and ok3 and (t_lower < t_upper):
                        # If t = 0 lies inside the feasible interval, then the least-squares/ base solution 
                        # is already strictly feasible, so keep it
                        if t_lower < 0.0 < t_upper:
                            interior_t = 0.0
                        # If both bounds are finite, we use the midpoint
                        elif math.isfinite(t_lower) and math.isfinite(t_upper):
                            interior_t = 0.5 * (t_lower + t_upper)
                        # If only a lower bound exists, choose any value strictly above the lower bound
                        # Here, we simply choose some strictly feasible point inside the interval --- + 1
                        elif math.isfinite(t_lower):
                            interior_t = t_lower + 1.0
                        # If only an upper bound exists, choose any value strictly below the upper bound
                        # Here, we simply choose some strictly feasible point inside the interval --- - 1
                        elif math.isfinite(t_upper):
                            interior_t = t_upper - 1.0
                        # If there are no finite bounds, every t is feasible for the positivity constraints,
                        # so keep the base solution.
                        else:
                            interior_t = 0.0

                        # Shift the base solution along the kernel-space direction, that remains a valid solution
                        interior_alpha1 = interior_alpha1 + interior_t * null_v1
                        interior_alpha2 = interior_alpha2 + interior_t * null_v2
                        interior_alpha3 = 1.0 - interior_alpha1 - interior_alpha2

                        # Check that the shifted solution is strictly inside the simplex
                        interior_feasible = (interior_alpha1 > 0.0) and (interior_alpha2 > 0.0) and (interior_alpha3 > 0.0)

                # After any interior candidate has been constructed or recovered, revalidate before accepting it as a valid interior candidate 
                # The candidate must be strictly inside the simplex and must still satisfy the interior stationarity system up to the numerical tolerance
                if interior_feasible:
                    interior_adjusted_solution = torch.tensor(
                        [interior_alpha1, interior_alpha2],
                        dtype=torch.float64,
                    )
                    # check if the adjusted solution still satisfy the interior stationarity system up to numerical tolerance
                    ...
                    # final acceptance check: the candidate must both satisfy stationarity and remain strictly inside the simplex
                    interior_feasible = (
                        interior_solve_success
                ...
                # If the interior candidate passed the final stationarity and simplex checks above, 
                # compute the quantities needed to evaluate its dual value
                if interior_feasible:
                    # We compute the quantities needed to evaluate the interior candidate's dual value
                    interior_lambda_combo_dot_w_t = interior_alpha1 * m1_dot_w_t + interior_alpha2 * m2_dot_w_t
                    # Compute ||lambda1 m1 + lambda2 m2||_{P_t^{-1}}^2 for the interior candidate
        ...
        # although in concave simplex dual subproblem, any feasible interior stationary point is globally optimal in exact arithmetic,
        # Here, for numerical safety, we still compare the valid interior candidate and all projected boundary candidates by their computed dual values
        if interior_feasible:
